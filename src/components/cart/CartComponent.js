...
const CartComponent = () => {
  ...
  const calculateFinalPrice = () => {
    ...
    setFinalPriceAfterDiscount(totalPrice - discount);
  };
  ...
  const handleVoucher = () => {
    if (voucher.trim().toUpperCase() === 'DISCOUNT10') {
      ...
      setFinalPriceAfterDiscount(finalPrice - discountAmount);
      Swal.fire({
        icon: 'success',
        title: 'Discount code applied',
        text: `You have saved ${discountAmount.toFixed(2)} RSD!`,
      });
    }
    else if (voucher.trim().toUpperCase() === 'DISCOUNT20') {
      const discountAmount = finalPrice * 0.2;
      setDiscount(discountAmount);
      setFinalPriceAfterDiscount(finalPrice - discountAmount);
      Swal.fire({
        icon: 'success',
      ...
      setDiscount(0);
      setFinalPriceAfterDiscount(finalPrice);
    }
  };
  ...
  const alertAreYouSureFinalOrder = () => {
    sumFinalPrice(); // Tính giá cuối cùng trước khi hiển thị cảnh báo

    // Tạo thông báo dựa trên trạng thái đăng nhập và mã giảm giá
    let textStr = localStorage.token == null
      ? `Final price is: ${finalPriceAfterDiscount.toFixed(2)} RSD! Would you like to confirm your order??`
      : `Final price after discount is: ${finalPriceAfterDiscount.toFixed(2)} RSD! Would you like to confirm your order??`;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // Gửi thông tin đơn hàng sau khi xác nhận
        sendFinalOrderAndHandleRepsonseFromServer({
          itemsFromCart,
          address,
          phoneNumber,
          finalPrice: finalPriceAfterDiscount // Sử dụng giá đã giảm
        });
      }
    });
  };
      ...
      <div className='cart-main-container'>
        ...
        {itemsFromCart.length !== 0 && <div className='cart-container'>
              ...
              return (
                <div className='cart-item-container'>
                  <div className='item-image-container'>
                    <img className='item-meal-pic' src={`http://localhost:8080/uploads/meal/${itemFromCart.mealName.toLowerCase().replace(/\s+/g, '_')}.jpg`} alt='' />
                  </div>
          ...
          <div className='summary'>
            <h1 className="cart-total">Total: {finalPrice} RSD</h1>
            <h1 className="discount">Discount: {discount.toFixed(2)} RSD</h1>
            <h1 className="discount">Amount Payable: {finalPriceAfterDiscount.toFixed(2)} RSD</h1>
            ...
            <div className="voucher-container">
              ...
              />
              <button className="voucher-button" onClick={handleVoucher}>Submit Voucher</button>
            </div>
          </div>
